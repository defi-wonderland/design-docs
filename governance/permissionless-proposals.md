...
4.	**Move to Vote via ProposalValidator**: The UI should reflect the available action. This action will be:
	...
	•	Disabled *Approve Proposal* Button: displayed if the proposal is not within the allowed submission window, within the distribution rate limit (if applicable), or meeting the proposal-type-specific thresholds. In this case, the unmet condition(s) should be shown.
...
- **Maintenance upgrade proposer trust**: Maintenance Upgrade Proposals proceed directly to a one-week veto period and cannot be executed on-chain via the optimistic module. There is a risk that an address approved to submit Maintenance Upgrades could misuse this path to avoid a full Token House vote when the change should be a Protocol/Governor Upgrade. Mitigations include clear scoping (“no material behavior change”), public accountability, and the ability for stakeholders to veto.
